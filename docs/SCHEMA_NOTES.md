...
- **`TimeEntry.leadId`** — STILL OPEN (a prior "resolved" note here was wrong; only `Expense.leadId` exists, as an unwired placeholder). Build path when wanted: add nullable `TimeEntry.leadId`, relax `matterId` to nullable with an app-enforced exactly-one-of-(matterId, leadId) invariant, wire the lead→matter conversion roll-forward, then mirror the matter Time tab on `/intake/[id]/time`. Relaxing `matterId` touches every query that assumes it non-null (billing WIP, matter tabs) — budget for that sweep.
