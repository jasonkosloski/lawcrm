...
51 models as of 2026-07-08: the initial 2026-04-24 snapshot plus everything added since (second table). When you add a model, add a row to the second table AND a changelog entry.
...
| CalendarEvent | Active | Google Calendar sync landed 2026-07-08 via the `CalendarEventSync` mapping model (no columns added here — sync state lives on the mapping + `EmailAccount`) |
...
| CalendarEventSync | 2026-07-08 | Maps one CalendarEvent to its copy on one Google account's `primary` calendar (uniques both ways: (eventId, accountId) + (accountId, googleEventId)); `googleUpdatedAt` anchors last-write-wins; cascades from both parents (see changelog) |
...
| 2026-07-08 | Google Calendar sync plumbing: `EmailAccount.grantedScopes` + `EmailAccount.calendarSyncToken`, new `CalendarEventSync` model (`@@unique([eventId, accountId])` + `@@unique([accountId, googleEventId])`, reverse relations `CalendarEvent.googleSyncs` / `EmailAccount.calendarSyncs`) | Two-way Google Calendar sync rides the Gmail connection (`EmailAccount` doubles as the Google account record). `grantedScopes` persists the token response's `scope` — Google's ACTUAL grant, not what we asked for — so calendar sync gates on capability (`hasCalendarScope`) and pre-calendar connections surface "Reconnect to enable calendar sync" instead of failing; written on connect AND reconnect. `calendarSyncToken` is the events.list incremental cursor (null = full windowed pull pending; a 410 GONE re-pulls the window and re-mints it). `CalendarEventSync` maps one CRM event to its copy on one account's `primary` calendar: the (eventId, accountId) unique means at most one copy per calendar, the (accountId, googleEventId) unique makes pull idempotent, and `googleUpdatedAt` (Google's `updated` at last sync) anchors last-write-wins. Cascade both ways — a mapping is bookkeeping, never a reason to keep either parent alive. Engines: pull `src/lib/google/google-calendar-sync.ts`, push `google-calendar-push.ts`, shared contract (echo marker in `extendedProperties.private`) `calendar-shared.ts`. |
