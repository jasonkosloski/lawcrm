...
- [~] **Calendar event create/edit form.** v1 ✓ shipped — full edit page at `/calendar/events/[eventId]/edit` covering title, type, start/end, location, Zoom URL, description, matter assignment + delete. **All-day events ✓** — toggle in the form swaps datetime → date inputs; action accepts date-only and stores as local midnight; week + month views render all-day chips as bars (not timed blocks). **Attendee picker ✓** — name + optional email, replace-all on save into CalendarAttendee. **Drag-and-drop reschedule ✓** — week-view chips drag to time slots (preserve duration when timed; default 2h when promoting from all-day) or to the all-day bar (collapse to all-day on that date). Generic DnD utilities at `src/lib/dnd/` (payload helpers + `useDropTarget` hook) so the same primitives feed the future kanban. Action `moveCalendarEvent` gated on `events.edit`. Still deferred: recurrence rules (RRULE expansion is a multi-day feature), keyboard reorder, drag-resize.
