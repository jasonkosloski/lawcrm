...
- [~] **SOL automation.** v1 ✓ shipped 2026-04-27 in two passes. **Pass 1:** `Matter.incidentDate` + `PracticeArea.statutePeriodDays` + `statuteSourceCitation` on the schema; practice-area edit form takes years/months/days inputs (packed into total days, 365/30/d legal convention) + a citation field; matter create + update auto-populate `statuteOfLimitationsDate` from `incidentDate + area.statutePeriodDays` when both are present (explicit SOL date wins). **Pass 2:** matter create + edit forms now expose the `Incident / accrual date` field with a hint that names the configured period + cite ("Period: 2 years · C.R.S. § 13-80-102. Leave Deadline blank to auto-compute."); the SOL card on the overview surfaces the cite + incident-date provenance line ("from incident MM/DD/YYYY · per C.R.S. § …"). Still pending: auto-create a critical `Deadline` row when SOL is set (the existing SOL ↔ Deadline mirror covers satisfied flips but not the initial set).
...
- [~] **Settlement distribution waterfall UI.** v1 ✓ shipped 2026-04-27. Settlement card on the matter Billing tab renders the gross → firm fee → advanced costs → liens → client net waterfall. Composer captures gross / firm fee % (the read layer recomputes the fee from percent at render so percentage edits propagate cleanly) / advanced costs / status (pending → approved → disbursed → closed). Lien composer + per-row negotiated/effective amount with `original` strikethrough when a negotiation reduced it. Permission keys: `matters.settlement.view` / `.edit` / `.manage_liens`. Audit-log entry on every settlement action. Still deferred: SettlementApproval workflow UI (4-step approval chain — schema is rich, no UI yet); printable / sign-off-ready distribution sheet for client signature; multi-settlement matters (today the assumption is one settlement per matter).
