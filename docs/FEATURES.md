...
- [x] **Matter detail — Billing tab (v1)** — Three-card KPI strip (WIP, Trust, Outstanding AR) + invoices table + WIP entries list + trust ledger. **Generate invoice from WIP** bundles every approved-and-unbilled time entry into a draft Invoice (auto YYYY-NNN numbering, 30-day default terms) and flips entries to `billed`. **Bundle as internal record** is the sibling action for contingency / pro-bono cases that close without a fee petition — same WIP-bundling mechanic, but the resulting Invoice has `kind="internal_record"` (born locked at status="paid" rendered as "Recorded"), is excluded from Outstanding AR, and the preview reads "Internal Record" / "For matter file" instead of "Invoice" / "Bill to". Reason captured into `Invoice.notes`. **Pay from trust** records an atomic four-leg op against an open client invoice (trust ledger row + trust balance decrement + invoice paidAmount increment + InvoicePayment row) — supports partial payments naturally; a "partial" chip surfaces on the row when 0 < paidAmount < totalAmount; ledger row shows "Payment to invoice X" as a deep link. **Record payment** is the sibling action for non-trust channels (check, ACH, cash, card, other) — opens a dialog that captures amount, method, date, reference, and memo, creates an `InvoicePayment` row, bumps `paidAmount`, and auto-flips the invoice to `paid` only when the payment fully covers the balance (partials leave status alone). The invoice preview surfaces every recorded payment in a "Payments received" section (date / source / description / reference / amount), with an inline "additional $X applied via Mark paid without a recorded transaction" note when `paidAmount` exceeds the sum of recorded payments (legacy / API mark-paid case). The Billing tab also carries a matter-level **Received payments** ledger (date / method / invoice link / reference / amount) showing every payment ever applied to any invoice on the matter — the matter-scoped slice of what will eventually be a firm-wide trust + operating ledger. Per-row status transitions (client: draft → sent → paid → void; internal: draft → paid → void). Void on either kind unlinks entries back to billable WIP. Trust composer adds deposits / disbursements / refunds with overdraw prevention. Letterhead-style preview pane opens on the right when an invoice is selected (sticky top bar with action buttons + close). `Matter.billingMode` chip surfaces the matter's billing flow (Client / Court-appointed / Fee petition / None); inherited from `PracticeArea.defaultBillingMode` on create, overridable per-matter via the matter edit form. Non-client modes show a "flow not implemented yet" banner and route through the traditional UX until each per-mode flow lands. Decimal math throughout. Deferred to v2: invoice line-item editing, expense tracking, PDF export, email send, court-voucher submission packets, fee-petition court exhibits, settlement waterfall, tax calculation, aging report.
...
- [x] **Settings — Roles** — First-class permission roles. Every firm seeds with two system roles: `Admin` (grants admin powers — anyone holding it can edit firm settings and manage team) and `default` (auto-assigned to every member, baseline for future granular permissions). Admins can create / rename / delete custom roles (system roles are locked). Today only `Admin` actually grants powers; custom roles are named buckets ready for granular permissions when those land. The old `User.isAdmin` boolean is gone — admin check is `userRoles: { some: { role: { name: "Admin" } } }` everywhere.
- [x] **Admin-only routes gated end-to-end** — `/settings/practice-areas` (+ `[id]`), `/settings/integrations`, `/settings/billing` all call `requireAdmin()` server-side; non-admins get a `<meta refresh>` bounce to the dashboard (Next.js 16 streaming-RSC redirect behavior). The settings nav also hides those entries for non-admins so the link never appears. Practice-area + stage CRUD actions all carry their own `requireAdmin()` call as defense-in-depth.
