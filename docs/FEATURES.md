...
- [x] **Matter detail — Billing tab (v1)** — Three-card KPI strip (WIP, Trust, Outstanding AR) + invoices table + WIP entries list + trust ledger. **Generate invoice from WIP** bundles every approved-and-unbilled time entry into a draft Invoice (auto YYYY-NNN numbering, 30-day default terms) and flips entries to `billed`. Per-row status transitions (draft → sent → paid → void; void unlinks entries back to WIP). Trust composer adds deposits / disbursements / refunds with overdraw prevention. Decimal math throughout — no Float drift on real money. Deferred to v2: invoice line-item editing, expense tracking, partial payments, PDF export, email send, settlement waterfall, tax calculation, aging report.
