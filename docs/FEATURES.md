...
- [x] **Settings — Roles** — First-class permission roles. Every firm seeds with two system roles: `Admin` (grants admin powers — anyone holding it can edit firm settings and manage team) and `default` (auto-assigned to every member, baseline for granular permissions). Admins can create / rename / delete custom roles (system roles are locked). The old `User.isAdmin` boolean is gone — admin check is `userRoles: { some: { role: { name: "Admin" } } }` everywhere. **Permissions matrix** on the same page lets admins toggle which permissions each role grants — Y-axis is every permission in the static catalog (grouped by category: Matters / Billing / Trust / Firm settings / Documents), X-axis is every role in the firm, intersection is a checkbox. Admin's column is rendered checked + locked (the runtime permission check treats Admin as "all granted" by name, so we don't materialize join rows for it). Toggles fire optimistically through `setRolePermissionAction`. The catalog ships with permissions covering matter team management, every billing state transition, trust transactions, firm-settings management, and document deletion. Today most authorization checks still go through `requireAdmin()`; the matrix is in place so admins can configure roles before specific code paths swap in `hasPermission(...)` checks.
