...
- [x] **Settings — Team** — Firm-scoped roster (admins float to the top, then active members alphabetically). Everyone sees the read-only list; admins get a kebab per row (Edit / Reset password) and an "Invite member" composer at the bottom. Server-side invariants: at-least-one-active-admin (rejects demote/deactivate that would zero out admins), can't-deactivate-yourself, and unique-email-on-invite. Reset-password and invite each generate a one-time URL-safe temp password the admin delivers out-of-band — replaced by magic-link email when delivery lands (Phase 2 of `docs/AUTH_PLAN.md`).
