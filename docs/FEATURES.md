...
- [x] **Settlement waterfall (v1)** — Settlement card on the matter Billing tab. Composer captures gross / firm fee % / advanced costs / status (pending → approved → disbursed → closed). Lien composer adds medical / subrogation / Medicare-Medicaid liens with negotiable amounts; per-row UI strikes through the original when a negotiated amount is set. Read layer recomputes firm fee from percent + client net from the deduction stack on every render so the numbers stay coherent if a lien gets renegotiated. Granular permissions: `matters.settlement.view` / `.edit` / `.manage_liens`. Audit-log entry on every settlement action. Deferred: SettlementApproval workflow UI (4-step approval chain), printable distribution sheet, multi-settlement matters.
